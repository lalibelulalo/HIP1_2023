    ...
    if kmer >= 4:
        ...
        if kmer >= 5:
            ...
            if kmer >= 6 or kmer == 41:
                ...
                if kmer >= 7 or kmer == 73:
                    heptanuc_genome = [genome[ii:ii+7] for ii, ch in enumerate(genome)]
                    heptanuc_genome_cnts = pd.Series(heptanuc_genome).value_counts().to_dict()
                    NucsKmers = (dinuc_genome_cnts, trinuc_genome_cnts, tetranuc_genome_cnts, pentanuc_genome_cnts, hexanuc_genome_cnts, heptanuc_genome_cnts)
                
                    if kmer == 8 or kmer == 82 or kmer == 83 or kmer == 61:
                        ...
                        NucsKmers = (dinuc_genome_cnts, trinuc_genome_cnts, tetranuc_genome_cnts, pentanuc_genome_cnts, hexanuc_genome_cnts, heptanuc_genome_cnts, octanuc_genome_cnts)
    ...
    if kmer >= 4:
        ...
        if kmer >= 5:
            ...
            if kmer >= 6 or kmer == 41:
                ...
                if kmer >= 7 or kmer == 73:
                    heptanuc_genome_cnts = NucsAndKmers[5]
                    ## Hexanucleotidos del palindromo en cuestion
                    heptanuc_pal = [pal[ii:ii+7] for ii, ch in enumerate(pal) if len(pal[ii:ii+7])==7 ]
                    HEPTANUCLEOTIDES = {x:(heptanuc_genome_cnts[x] if x in heptanuc_genome_cnts else 0) for x in heptanuc_pal}
                    KNUCLEOTIDES = (DINUCLEOTIDES,TRINUCLEOTIDES,TETRANUCLEOTIDES,PENTANUCLEOTIDES,HEXANUCLEOTIDES,HEPTANUCLEOTIDES)
                
                    if kmer == 8 or kmer == 82 or kmer == 83 or kmer == 61:
                        octanuc_genome_cnts = NucsAndKmers[6]
                        ...
                        KNUCLEOTIDES = (DINUCLEOTIDES,TRINUCLEOTIDES,TETRANUCLEOTIDES,PENTANUCLEOTIDES,HEXANUCLEOTIDES,HEPTANUCLEOTIDES,OCTANUCLEOTIDES)
        return(KNUCLEOTIDES)
...
def Heptanuc1NtNoPalGen(Nucs):
    pals1 = [Nucs[j]+Nucs[k]+Nucs[l]+Nucs[-l]+Nucs[-k]+Nucs[-j]+Nucs[-i] for i in range(1,(len(Nucs))) for j in range(1,(len(Nucs))) for k in range(1,(len(Nucs))) for l in range(1,(len(Nucs)))]
    pals2 = [Nucs[i]+Nucs[j]+Nucs[k]+Nucs[l]+Nucs[-l]+Nucs[-k]+Nucs[-j] for i in range(1,(len(Nucs))) for j in range(1,(len(Nucs))) for k in range(1,(len(Nucs))) for l in range(1,(len(Nucs)))]
    HeptNoPal_all = [pals1,pals2]
    HeptNoPal_all = np.unique([j for i in HeptNoPal_all for j in i])
    pals = HeptNoPal_all
    return(pals)

def HeptanucGen(Nucs):
    Hept_all = [Nucs[i]+Nucs[j]+Nucs[k]+Nucs[l]+Nucs[-k]+Nucs[-j]+Nucs[-i] for i in range(1,(len(Nucs))) for j in range(1,(len(Nucs))) for k in range(1,(len(Nucs))) for l in range(1,(len(Nucs)))]
    pals = Hept_all
    return(pals)
...
def HIP1Like1NtNoPalGen(Nucs):
    hip_like_1 = [Nucs[i]+"CGATCGC" for i in range(1,(len(Nucs)))]
    hip_like_2 = ["G"+Nucs[i]+"GATCGC" for i in range(1,(len(Nucs)))]
    hip_like_3 = ["GC"+Nucs[i]+"ATCGC" for i in range(1,(len(Nucs)))]
    hip_like_4 = ["GCG"+Nucs[i]+"TCGC" for i in range(1,(len(Nucs)))]
    hip_like_5 = ["GCGA"+Nucs[i]+"CGC" for i in range(1,(len(Nucs)))]
    hip_like_6 = ["GCGAT"+Nucs[i]+"GC" for i in range(1,(len(Nucs)))]
    hip_like_7 = ["GCGATC"+Nucs[i]+"C" for i in range(1,(len(Nucs)))]
    hip_like_8 = ["GCGATCG"+Nucs[i] for i in range(1,(len(Nucs))) for j in range(1,(len(Nucs)))]
    hip_like_all = [hip_like_1,hip_like_2,hip_like_3,hip_like_4,hip_like_5,hip_like_6,hip_like_7,hip_like_8]
    HIP1Like1NtNoPal_all = np.unique([j for i in hip_like_all for j in i])
    pals = HIP1Like1NtNoPal_all
    return(pals)
