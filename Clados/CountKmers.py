...
from CountFunctions import HeptanucGen
from CountFunctions import Heptanuc1NtNoPalGen
from CountFunctions import OctanucGen
from CountFunctions import HIP1Like2NtPal
from CountFunctions import HIP1Like1NtNoPalGen
import os
import glob
import time
import re
...
def CountKmers(GENOMES_PATH,KMERS,ORDER):
    ...
    if KMERS == 4:
        ...
        pals = HeptanucGen(Nucs)
        kmer = 'Heptanuc'
    elif KMERS == 73:
        pals = Heptanuc1NtNoPalGen(Nucs)
        kmer = 'Heptanuc1ntNoPal'
    elif KMERS == 8:
        pals = OctanucGen(Nucs)
        kmer = 'Octanuc'
    elif KMERS == 82:
        pals = HIP1Like2NtPal(Nucs)
        kmer = 'OctanucHIP2ntPal'
    elif KMERS == 83:
        pals = HIP1Like1NtNoPalGen(Nucs)
        kmer = 'OctanucHIP1ntNoPal'
    else:
    ...
    output_file = str("_".join (['Markov_count',kmer,model,'.txt'])) #genomepath,date
    ...
    for GenomeFile in genomes:
        ...
        for pal in pals:
            ...
            elif KMERS == 7 or KMERS == 73:
                DINUCLEOTIDES,TRINUCLEOTIDES,TETRANUCLEOTIDES,PENTANUCLEOTIDES,HEXANUCLEOTIDES,HEPTANUCLEOTIDES = PalKmerCounts(pal,KMERS,NucsKmers)
                KNUCS = HEPTANUCLEOTIDES
            elif KMERS == 8 or KMERS == 82 or KMERS == 83 or KMERS == 61:
