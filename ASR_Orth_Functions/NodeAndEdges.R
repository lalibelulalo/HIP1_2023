...
Create_Transition_Table_No_Fit <- function(SitesTable,EvolutionModel,Method,Phylogeny,OrthoPath,OutName) {
  ## Primero cargo Orthologues_Palindrome_sites.txt del paso anterior.
  Sites <- read.table(SitesTable, sep = "\t", header = TRUE)
  ...
  DELECIONES = as.data.frame(DELECIONES)
  DELECIONES = DELECIONES[order(DELECIONES$deleciones,decreasing = TRUE),]
  write.table(DELECIONES,file=paste0(OutName,".DELECIONES.txt"),sep="\t",row.names = FALSE,col.names = TRUE)
  write.table(DF,file=paste0(OutName,".TRANSICIONES.txt"),sep="\t",row.names = FALSE,col.names = TRUE)
  write.table(LINKS,file=paste0(OutName,".TRANSICIONES_DIRECCION.txt"),sep="\t",row.names = FALSE,col.names = TRUE)
  #TABLE1 = as.data.frame(DF)
  #TABLE2 = as.data.frame(LINKS)
  #return(list(nodes,edges,TABLE2))
}
#---------------------------------------------------------------------------------------------------------------------------------------
Create_Network <- function(Transitions,MinimumCounts,Loops){
  DF = Transitions
...
#---------------------------------------------------------------------------------------------------------------------------------------
CreateCodonMutationsTable <- function(Tree,Second,TaxonNumber,RF,Root) {
