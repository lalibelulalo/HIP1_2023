...
library(ggplot2)
library(ggtree)
Codon_Mutation_Node_Pie_Charts3 <- function(Tree,SppPath,HIP1NodeStatus,Palindrome,TreePlot,SPP) {
  ...
  for (RF in RFFiles){
    ...
    if (RF==3){
      tableRF = tableRF[ , c(2,43,44,47)]#,45
    }else{
      tableRF = tableRF[ , c(2,41,42,45)]#,43
    }
    ...
    ColNames  <- c("Transitions","Transversions","Deletions","node") #"NoMutations",
  ...
  for (RF in RFFiles) {
    ...
    for (i in 1:length(RF_list[[RF]][,1])) {
      data <- data.frame(
        group=c(ColNames[1:3]),
        value=as.numeric(as.vector(RF_list[[RF]][1:3][i,]))
      )
      AcualNode <- RF_list[[RF]][4][i,]
